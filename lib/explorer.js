...
module.exports = {
  ...
  /**
   * Check object does not exists in array
   */
...
//      if (vout[i].scriptPubKey.type != 'nonstandard' && vout[i].scriptPubKey.type != 'nulldata') { 
        ...
        module.exports.is_unique(arr_vout, vout[i].scriptPubKey.addresses[0], function(unique, index) {
          if (unique == true) {
            ...
            module.exports.convert_to_satoshi(parseFloat(vout[i].value), function(amount_sat){
              arr_vout.push({addresses: vout[i].scriptPubKey.addresses[0], amount: amount_sat, n: vout[i].n});
              loop.next();
            });
          } else {
            // already exists
  ...
  /**
   * Calculate  amount of address, get address of this vin transaction
   * @parameter input vin[i] of tx
   * @parameter vout vout of tx 
   */
  get_input_addresses: function(input, vout, cb) {
    var addresses = [];
    // If input is coinbase, we calculate total value of vout
    /*"vin": [{
		"coinbase": "0435e9494d0136062f503253482f",
		"sequence": 4294967295
	}]
	*/
    if (input.coinbase) {
      ...
      module.exports.syncLoop(vout.length, function (loop) {
        ...
        addresses.push({hash: 'coinbase', amount: amount, n:-1});
        return cb(addresses);
      });
    } else {
    	/*"vin": [
    	    	{
    	    		"txid": "42f7b3a48ec75bd12d63a193afc09c4c6f3569415ccad2a95d18925e8b716e11",
    	    		"vout": 0,
    	    		"scriptSig": {
    	    			"asm": "xxx",
    	    			"hex": "xxx"
    	 ...
    	 */
    	// We have to get transaction of input to calculate total value
            ...
            if (tx.vout[i].n == input.vout) {
              //module.exports.convert_to_satoshi(parseFloat(tx.vout[i].value), function(amount_sat){
            	if(tx.vout[i].scriptPubKey.addresses){
            		addresses.push({hash: tx.vout[i].scriptPubKey.addresses[0], amount:tx.vout[i].value, n:input.vout});
            	} else {
            		addresses.push({hash: 'coinbase', amount:tx.vout[i].value, n:-1});
            	}
          ...
          module.exports.is_unique(arr_vin, addresses[0].hash, function(unique, index) {
            if (unique == true) {
              module.exports.convert_to_satoshi(parseFloat(addresses[0].amount), function(amount_sat){
                arr_vin.push({addresses:addresses[0].hash, amount:amount_sat, n:addresses[0].n, prev_txid: tx.vin[i].txid});
                loop.next();
              });
            } else {
              module.exports.convert_to_satoshi(parseFloat(addresses[0].amount), function(amount_sat){
